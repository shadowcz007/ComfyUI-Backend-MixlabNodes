...
def get_model_dir(m):
    try:
        fp=folder_paths.get_folder_paths(m)[0]
        return fp
    except:
        fp=os.path.join(folder_paths.models_dir, m)
    # fp=fp.replace("\\", "/")
    return fp

models_dir = folder_paths.models_dir
REACTOR_MODELS_PATH = get_model_dir("reactor")
...
ultralytics_p=get_model_dir("ultralytics")

if "ultralytics_bbox" not in folder_paths.folder_names_and_paths:
    add_folder_path_and_extensions("ultralytics_bbox", [os.path.join(ultralytics_p, "bbox")], folder_paths.supported_pt_extensions)
if "ultralytics_segm" not in folder_paths.folder_names_and_paths:   
    add_folder_path_and_extensions("ultralytics_segm", [os.path.join(ultralytics_p, "segm")], folder_paths.supported_pt_extensions)

if "sams" not in folder_paths.folder_names_and_paths:
    add_folder_path_and_extensions("sams", [get_model_dir("sams")], folder_paths.supported_pt_extensions)
...
def get_restorers():
    models_path = os.path.join(get_model_dir("facerestore"), "*")
    models = glob.glob(models_path)
